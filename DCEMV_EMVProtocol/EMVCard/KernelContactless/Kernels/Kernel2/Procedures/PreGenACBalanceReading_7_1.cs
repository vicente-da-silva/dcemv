...
{
    public static class PreGenACBalanceReading_7_1
    {
        internal static SignalsEnum PreGenACBalanceReading(Kernel2Database database, KernelQ qManager, CardQ cardQManager)
        {
            ...
            SignalsEnum result = SignalsEnum.WAITING_FOR_PRE_GEN_AC_BALANCE;
            while (result == SignalsEnum.WAITING_FOR_PRE_GEN_AC_BALANCE)
            {
                CardResponse cardResponse = cardQManager.DequeueFromOutput(false);
                result = State_16_WaitingForPreGenACBalance_7_2.Execute_State_16_WaitingForPreGenACBalance(database, qManager, cardQManager);
            }
